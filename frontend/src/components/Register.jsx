...
const Register = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          />
          <p className="mt-1 text-sm text-gray-500">Upload a clear image of your driving license (max 5MB)</p>
        </div>

        <button
          type="submit"
          className={`w-full py-3 rounded-lg transition-colors ${
            isPhoneVerified ? "bg-blue-600 text-white hover:bg-blue-700" : "bg-gray-400 text-white cursor-not-allowed"
          }`}
          disabled={loading || !isPhoneVerified}
        >
      ...
      {loading && (
        <Loader
          message={uploadProgress > 0 ? `Uploading: ${uploadProgress}% complete` : "Processing your registration..."}
        />
      )}
